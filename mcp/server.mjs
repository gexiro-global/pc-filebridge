...
var RootConfigSchema = z2.object({
  id: z2.string().regex(/^[a-z][a-z0-9-]{0,31}$/),
  label: z2.string().min(1).max(80),
  path: z2.string().min(1),
  read: z2.boolean(),
  create: z2.boolean(),
  host_id: z2.string().regex(/^[a-z][a-z0-9-]{0,63}$/).optional(),
  volume_id: z2.string().regex(/^vol-[a-f0-9]{16}$/).optional(),
  current_drive_letter: z2.string().regex(/^[A-Z]$/).optional(),
  filesystem: z2.string().min(1).max(32).optional(),
  bus_type: z2.string().min(1).max(64).optional(),
  online: z2.boolean().optional(),
  auto_discovered: z2.boolean().optional()
}).strict();
var LimitsSchema = z2.object({
  maxReadBytes: z2.number().int().min(1024).max(1024 * 1024).default(256 * 1024),
  maxWriteBytes: z2.number().int().min(1024).max(4 * 1024 * 1024).default(1024 * 1024),
  maxDirectoryEntries: z2.number().int().min(1).max(1e3).default(250),
  maxSearchResults: z2.number().int().min(1).max(250).default(100),
  maxSearchEntries: z2.number().int().min(100).max(25e3).default(5e3)
}).strict();
var FileBridgeConfigSchema = z2.object({
  version: z2.literal(1),
  roots: z2.array(RootConfigSchema).min(1).max(26),
  limits: LimitsSchema
}).strict();
...
var SSH_PRIVATE_KEY_FILENAME = /^id_(?:rsa|dsa|ecdsa|ed25519)(?:[._-].*)?$/;
var WINDOWS_RESERVED = /^(con|prn|aux|nul|com[1-9]|lpt[1-9])(?:\..*)?$/i;
var DRIVE_ROOT_OPT_IN = "I_ACCEPT_FULL_DRIVE_ACCESS_RISK";
var MAX_CANONICALIZATION_ENTRIES = 25e3;
var FileBridgePolicy = class _FileBridgePolicy {
  ...
  roots;
  limits;
  static async fromFile(configPath, driveRootOptIn = process.env.FILEBRIDGE_ALLOW_DRIVE_ROOT) {
    let raw;
    try {
    ...
    return _FileBridgePolicy.fromConfig(FileBridgeConfigSchema.parse(parsed), driveRootOptIn);
  }
  static async fromConfig(config, driveRootOptIn) {
    ...
    return new _FileBridgePolicy(roots, validated.limits);
  }
  listRoots() {
    return [...this.roots.values()].map((root) => ({
      ...
      ...root.host_id === void 0 ? {} : { host_id: root.host_id },
      ...root.volume_id === void 0 ? {} : { volume_id: root.volume_id },
      ...root.current_drive_letter === void 0 ? {} : { current_drive_letter: root.current_drive_letter },
      ...root.filesystem === void 0 ? {} : { filesystem: root.filesystem },
      ...root.bus_type === void 0 ? {} : { bus_type: root.bus_type },
      ...root.online === void 0 ? {} : { online: root.online },
      ...root.auto_discovered === void 0 ? {} : { auto_discovered: root.auto_discovered }
    }));
  }
  async statPath(rootId2, relativePath2) {
  ...
  async searchNames(rootId2, query, startPath = "") {
    ...
    const start = await this.resolveForRead(rootId2, startPath);
    ...
    while (queue.length > 0 && scanned_entries < this.limits.maxSearchEntries && matches.length < this.limits.maxSearchResults) {
      ...
      const current = await this.resolveForRead(rootId2, currentLogical);
      let directory;
      try {
  ...
  async createDirectory(rootId2, relativePath2) {
    ...
    return { root_id: rootId2, path: logicalPath(relativePath2), created: true, kind: "directory" };
  }
  async createTextFile(rootId2, relativePath2, content) {
    const resolved = await this.resolveForCreate(rootId2, relativePath2);
    if (!resolved.normalizedRelative) throw new PolicyError("ROOT_WRITE_BLOCKED", "The configured root cannot be created or replaced.");
    if (content.includes("\0")) throw new PolicyError("BINARY_CONTENT_BLOCKED", "Only UTF-8 text content is accepted.");
  ...
  getRoot(rootId2, operation) {
    const root = this.roots.get(rootId2);
  ...
  async resolveForRead(rootId2, relativePath2) {
    const root = this.getRoot(rootId2, "read");
    const normalizedRelative = validateRelativePath(relativePath2);
    const authorized = await authorizeExistingPath(root, normalizedRelative);
    return { root, normalizedRelative, ...authorized };
  }
  async resolveForCreate(rootId2, relativePath2) {
    const root = this.getRoot(rootId2, "create");
    const normalizedRelative = validateRelativePath(relativePath2);
...
function validateRelativePath(input) {
  if (typeof input !== "string" || input.includes("\0")) throw new PolicyError("INVALID_PATH", "Path is invalid.");
...
function validatePathComponent(part) {
...
function validatePathComponentSyntax(part) {
...
function isBlockedPart(part) {
  const lower = part.toLocaleLowerCase();
  return BLOCKED_SEGMENTS.has(lower) || BLOCKED_FILENAMES.has(lower) || SSH_PRIVATE_KEY_FILENAME.test(lower) || lower.startsWith(".env") || BLOCKED_EXTENSIONS.has(path.extname(lower));
}
async function authorizeConfiguredRoot(configuredPath) {
...
async function authorizeExistingPath(root, normalizedRelative) {
  let rootInfo;
  try {
...
async function authorizeStoredComponents(start, startInfo, suppliedParts, boundary, enforceSensitiveNames = true, enforceAlternateFromIndex = 0) {
  let current = start;
...
function assertStableIdentity(info) {
...
async function assertUnchanged(target, expected) {
  let current;
  try {
...
function sameFileIdentity(left, right) {
  ...
  if (leftDev === null || leftIno === null || rightDev === null || rightIno === null || leftDev < 0n || rightDev < 0n || leftIno <= 0n || rightIno <= 0n) return false;
...
async function main() {
  ...
  server.registerTool(
    "stat_path",
    {
      ...
      inputSchema: z3.object({ root_id: rootId, relative_path: relativePath }),
      annotations: readAnnotations
    },
  ...
  server.registerTool(
    "search_file_names",
    {
      ...
      description: "Recursively search names, not file contents, below an allowed relative path. Results and traversal are strictly bounded.",
      inputSchema: z3.object({
        root_id: rootId,
  ...
  server.registerTool(
    "create_directory",
    {
      ...
      description: "Create exactly one new directory below an allowed root. The parent must exist. Existing paths are never replaced or reused.",
      inputSchema: z3.object({ root_id: rootId, relative_path: z3.string().min(1).max(4096) }),
  ...
  server.registerTool(
    "create_text_file",
    {
      ...
      description: "Create one new UTF-8 text file using OS-level exclusive create mode. If the target exists, the call fails without changing it.",
      inputSchema: z3.object({
        root_id: rootId,
        relative_path: z3.string().min(1).max(4096),
        content: z3.string().max(1024 * 1024)
      }),
      annotations: createAnnotations
    },
...
function safeError(error) {
  ...
  if (error instanceof z3.ZodError) return { code: "INVALID_INPUT", message: "Tool input did not match the required schema." };
